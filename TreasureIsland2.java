...
public class TreasureIsland2 {
	  ...
	  public int updateMatrix(int[][] matrix){
	      ...
	      for(int i = 0; i < R; ++i){
	          for(int j = 0; j < C; ++j){
	              if(matrix[i][j] == 0){
	                  queue.add(new int[]{i, j}); matrix[i][j] = 2;
	              }
	          }
	      }
	      while(!queue.isEmpty()){
	          ...
	          for(int i = 0; i < size; ++i){
	              ...
	              for(int[] dir: dirs){
	                  ...
	                  if(x < 0 || x >= R || y < 0 || y >= C || matrix[x][y] == 2) continue;
	                  if(matrix[x][y] == 1) return steps;
	...
	*You have a map that marks the locations of treasure islands. Some of the map area has jagged rocks and dangerous reefs. Other areas are safe to sail in. There are other explorers trying to find the treasure. So you must figure out a shortest route to one of the treasure islands.
	...
	Assume the map area is a two dimensional grid, represented by a matrix of characters. You must start from one
	 of the starting point (marked as S) of the map and 
	...
	**/
	    ...
	    public static void main(String[] args) {
	    	TreasureIsland2 main=new TreasureIsland2();
